    ...
    >
      <div className="flex justify-between items-start">
        <div className="flex-1">
          {type === 'books' && (
            <>
              <h4 className="text-lg font-semibold text-gray-900 mb-2">{reservation.title}</h4>
              <p className="text-sm text-gray-600 mb-2">Autor: {reservation.author}</p>
              <p className="text-sm text-gray-600 m-0">Fecha de devolución: {reservation.returnDate}</p>
            </>
          )}
          {type === 'cubicles' && (
            <>
              <h4 className="text-lg font-semibold text-gray-900 mb-2">Cubículo {reservation.number}</h4>
              <p className="text-sm text-gray-600 mb-2">Fecha: {reservation.date}</p>
              <p className="text-sm text-gray-600 m-0">Horario: {reservation.time}</p>
            </>
          )}
          {type === 'rooms' && (
            <>
              <h4 className="text-lg font-semibold text-gray-900 mb-2">{reservation.name}</h4>
              <p className="text-sm text-gray-600 mb-2">Fecha: {reservation.date}</p>
              <p className="text-sm text-gray-600 m-0">Horario: {reservation.time}</p>
            </>
          )}
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-gray-600 m-0">Gestiona todas tus reservas de libros, cubículos y salas</p>
      </div>
      {/* Tab */}
      ...
      <div className="flex flex-col gap-4">
        {mockReservations[activeTab as keyof MockReservations]?.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-12">
            ...
            <h3 className="text-xl font-semibold text-gray-700 mb-2">No tienes reservas de {tabs.find(t => t.id === activeTab)?.label.toLowerCase()}</h3>
            <p className="text-gray-500">Realiza una nueva reserva para verla aquí</p>
          </div>
        )}
      </div>
    </div>
